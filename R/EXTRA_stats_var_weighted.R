...
for (i in c("alpha_diversity", "beta_similarity", "beta_structure")) {
  models$one_fac[[i]] <-
    map(c(1, v[-1]), function(x) {
      ...
      w <- NULL
      if(i == "alpha_diversity") w <- 1/dat$var_alpha
      if(i == "beta_similarity") w <- 1/sqrt(dat$var_beta_similarity)
      if(i == "beta_structure") w <- 1/sqrt(dat$var_beta_structure)
